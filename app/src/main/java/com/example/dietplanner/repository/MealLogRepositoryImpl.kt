...
class MealLogRepositoryImpl @Inject constructor(
    ...
    override suspend fun insert(log: MealLog) {
        dao.insert(log)
    }

    override suspend fun update(log: MealLog) {
        dao.update(log)
    }

    override suspend fun delete(log: MealLog) {
        dao.delete(log)
    }

    override suspend fun deleteMealLogById(mealLogId: Int, userId: Int) {
        dao.deleteMealLogById(mealLogId, userId)
    }

    override suspend fun getMealLogsByUser(userId: Int): List<MealLog> {
        return dao.getMealLogsByUser(userId)
    }
